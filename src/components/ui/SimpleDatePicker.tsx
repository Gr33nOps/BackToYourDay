...
export function SimpleDatePicker({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "w-full rounded-xl border border-surface-border bg-surface/90 p-4 transition-colors duration-200 hover:border-foreground-dim/40",
        className
      )}
    >
      <div className="flex items-center justify-between px-0.5 mb-3">
        <span className="text-[11px] font-mono font-semibold uppercase tracking-widest text-accent flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-[1.65fr_0.95fr_1.15fr] gap-2 sm:gap-2.5 items-end">
        {/* Month Selector */}
        ...
        <div className="relative group min-w-0">
          ...
          <div className="relative">
            <select
              ...
              className="w-full appearance-none rounded-lg border border-surface-border bg-canvas hover:border-foreground-dim/50 pl-2 sm:pl-2.5 pr-5 sm:pr-6 py-2.5 text-xs sm:text-sm font-semibold text-foreground tracking-tight transition-colors focus:border-accent focus:outline-none focus:ring-1 focus:ring-accent cursor-pointer"
            >
              {days.map((d) => (
